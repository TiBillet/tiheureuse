...
# ---- Vanne ----
class Vanne:
    def __init__(self): self._o=False; self.lock=threading.Lock(); self._last_open_ts=0
    def ouvrir(self):
        with self.lock:
            GPIO.output(PIN_VANNE, GPIO.HIGH)
            self._o=True
            self._last_open_ts = time.monotonic()
        compteur.enable()
    def fermer(self):
        with self.lock:
            # respect min ON pour éviter clignotement violent sur relais/valve
            if self._o and (time.monotonic() - self._last_open_ts) < (RFID_MIN_OPEN_MS/1000.0):
                time.sleep((RFID_MIN_OPEN_MS/1000.0) - (time.monotonic() - self._last_open_ts))
            GPIO.output(PIN_VANNE, GPIO.LOW)
            self._o=False
        compteur.disable()
...
class CompteurDebit:
    def __init__(self, k):
        ...
        GPIO.add_event_detect(PIN_DEBIT, GPIO.FALLING, callback=self._pulse, bouncetime=BOUNCE_MS)
...
compteur = CompteurDebit(FACTEUR_CALIBRATION)
