...
class RfidSession(models.Model):
    ...
    label_snapshot = models.CharField(
        max_length=100, blank=True, help_text="Copie du label au début"
    )
    ...
    liquid_label_snapshot = models.CharField(
        max_length=100, blank=True, help_text="Copie du nom du liquide au début"
    )
    ...
    def __str__(self):
        ...
        return f"{self.tireuse_bec.name}:{self.uid} [{status}] {self.started_at:%Y-%m-%d %H:%M:%S}"
